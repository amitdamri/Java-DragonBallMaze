...
public class ServerStrategySolveSearchProblem implements IServerStrategy {
    ...
    public void serverStrategy(InputStream inFromClient, OutputStream outToClient) {
        try {
            ...
            Server.LOG.info("The solution was solved by the " + Configurations.getSearchingAlgorithm().getName() + " algorithm");
            Server.LOG.info("Solution for Maze is: ");
            for (int i = 1; i <= solPath.size(); i++)
                Server.LOG.info(String.format("%s. %s", i, solPath.get(i - 1)));
